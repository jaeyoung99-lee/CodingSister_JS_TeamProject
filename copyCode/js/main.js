...
const handleSearch = (inputType) => {
    searchPlaces(inputType, placesSearchCallback);
}
...
const searchPlaces = (inputType, callback) => {
    const inputId = inputType === 'start' ? 'search-start' : 'search-end';
    ...
    ps.keywordSearch(keyword, (data, status) => callback(data, status, inputType));
}
